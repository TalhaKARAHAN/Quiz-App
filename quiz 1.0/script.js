...
// JSONPlaceholder'dan soruları almak için asenkron bir fonksiyon
async function fetchQuestions() {
  try {
    ...
    // İlk 10 soruyu al ve her bir soruyu formatla
...
function generateOptionsWithShortWords(questionText) {
  ...
  return shuffledOptions.map((option, index) => `${option} - ${getShortWord(questionText, index)}`);
}

// Belirli bir uzunluktaki kelimeyi almak için yardımcı fonksiyon
function getShortWord(text, index) {
  const words = text.split(' ').filter(word => word.length <= 8);
